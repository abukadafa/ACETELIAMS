...
const SystemSettings: React.FC = () => {
    ...
    const fetchSettings = async () => {
        try {
            const data = (await api.get('/settings')) as SystemSettingsData;
            setSettings(data);
        } catch (error) {
    ...
    const saveSettings = async (key: string, value: any) => {
        try {
            ...
            setTestResult({ type: 'error', message: error?.message || 'Failed to save settings' });
        } finally {
            setSaving(false);
        }
    };
    ...
    const testSMTP = async () => {
        setTestResult(null);
        try {
            const data: any = await api.post('/settings/test-smtp', { smtpConfig: settings.smtp });
            setTestResult({
                type: 'success',
                message: data?.message || 'SMTP test completed',
            });
        } catch (error: any) {
            setTestResult({ type: 'error', message: error?.message || 'SMTP test failed' });
        }
    };
