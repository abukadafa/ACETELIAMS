...
export class EmailService {
    ...
    private getDefaultTemplate(eventName: string) {
        const defaults: { [key: string]: { subject: string, body: string } } = {
            'welcome': {
                subject: 'Welcome to ACETEL IAMS',
                body: 'Hello {{name}},\n\nWelcome to the ACETEL Integrated Academic Management System. Your account is now active.'
            },
            'password_reset': {
    ...
    async sendEmail(to: string, eventName: string, data: any) {
        try {
            ...
            const mailOptions = {
                from: (await configService.getSMTPConfig()).from || 'no-reply@acetel.edu.ng',
                to,
